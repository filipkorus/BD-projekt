CREATE OR REPLACE FUNCTION blokuj_usuwanie_aukcji_i_samochodow_po_sprzedazy()
    RETURNS TRIGGER AS $$
BEGIN
    IF TG_TABLE_NAME = 'aukcje' AND OLD.sprzedane = TRUE AND OLD.kupione_przez_uid IS NOT NULL THEN
...
CREATE TRIGGER blokuj_usuwanie_aukcji_po_sprzedazy_trigger
    BEFORE DELETE ON aukcje
    FOR EACH ROW
EXECUTE FUNCTION blokuj_usuwanie_aukcji_i_samochodow_po_sprzedazy();

CREATE TRIGGER blokuj_usuwanie_samochodow_po_sprzedazy_trigger
