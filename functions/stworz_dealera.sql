CREATE OR REPLACE FUNCTION stworz_dealera(
    _login VARCHAR,
    _haslo VARCHAR,
    _imie VARCHAR,
    _nazwisko VARCHAR,
...
BEGIN
    BEGIN
        ...
        VALUES (_imie, _nazwisko, _login, _email, _nr_tel, _nr_tel_publiczny, 'dealer');

        -- stworz uzytkownika psql
        EXECUTE 'CREATE USER ' || quote_ident(_login) || ' WITH PASSWORD ' || quote_literal(_haslo);

        -- dodaj nowego dealera do grupy uprawnien dealer_group
        EXECUTE 'GRANT dealer_group TO ' || quote_ident(_login);
